...
if (!gotTheLock) {
    ...
    ipcMain.handle('modal-start-chat', async (event, modelName, initialMessageRaw) => {
        ...
        // create new chat
        const newChatResp = await apiFetch(cfg, '/api/v1/chats/new', {
            method: 'POST',
            body: JSON.stringify({
                chat: {
        ...
        const completionResp = await apiFetch(cfg, '/api/chat/completions', {
            method: 'POST',
            body: JSON.stringify({
                model: modelName,
                messages: [{
        ...
        const assistantMessage = completionResp?.choices?.[0]?.message?.content;
    ...
    function checkOpenWebUI(ip, port) {
        return new Promise(resolve => {
            const req = http.get({ host: ip, port, path: '/api/version', timeout: 1000 }, res => {
                let data = '';
                res.on('data', chunk => data += chunk);
                res.on('end', () => {
                    try {
                        const json = JSON.parse(data);
                        if (json.version && compareVersion(json.version, "0.6.5") >= 0) {
    ...
    async function scanNetworkForOpenWebUI() {
        ...
        for (let i = 0; i < tasks.length; i += CONCURRENCY) {
            ...
            const found = await Promise.all(slice.map(t => checkOpenWebUI(t.ip, t.port)));
